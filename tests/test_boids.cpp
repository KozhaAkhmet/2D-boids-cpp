...
int main() {
	...
	Fish cursorFish(col_radius, speed, SimMath::icon_size, 0.0001,
	...
	while (window.isOpen()) {
		...
		if(fishes_nearby_dummy.size() > 0){
			...
			SimMath::cohesion(dummy_ptr, fishes_nearby_dummy);
			SimMath::applyModifiedDirection(dummy_ptr);
		}

		if(fishes_nearby_dummy2.size() > 0){
			SimMath::separation(dummy_ptr2, fishes_nearby_dummy2);
			SimMath::alignment(dummy_ptr2, fishes_nearby_dummy2);
			SimMath::cohesion(dummy_ptr2, fishes_nearby_dummy2);		
		...
		if(fishes_nearby_cursor.size() > 0){
			...
			SimMath::cohesion(cursor_ptr, fishes_nearby_cursor);
